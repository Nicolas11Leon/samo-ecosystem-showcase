...
![Security](https://img.shields.io/badge/Security-freeRASP_%26_RLS-critical?style=for-the-badge&logo=shield)
...
- [Core Engineering Highlights](#-core-engineering-highlights)
  ...
  - [3. Enterprise POS & Financial Reconciliation Engine](#3-enterprise-pos--financial-reconciliation-engine)
  - [4. Tournament Engine & Bracket Generation](#4-tournament-engine--bracket-generation)
  - [5. Dynamic ELO & League Points (LP) Matchmaking](#5-dynamic-elo--league-points-lp-matchmaking)
  - [6. DevSecOps: Mobile App Hardening (RASP) & Multi-Tenant RLS](#6-devsecops-mobile-app-hardening-rasp--multi-tenant-rls)
- [Architectural Blueprint](#-architectural-blueprint)
- [Tech Stack & Dependencies](#-tech-stack--dependencies)
- [Quality Assurance & Testing Strategy](#-quality-assurance--testing-strategy)
- [Author & Engineering Inquiries](#-author--engineering-inquiries)

---

## 🌟 Executive Overview

**SAMO** is a unified multiplatform solution that bridges the gap between competitive sports players and sports facility administrators. The platform solves two core engineering domains:

1. **High-Engagement Player & League Experience:** Real-time court bookings, open-match matchmaking, dynamic ELO-based ladder rankings, consensus-driven score voting, and automated multi-stage tournament brackets.
...
graph TD
    ...
    subgraph Infrastructure ["☁️ Cloud & Hardware Infrastructure"]
...
* **Functional Error Handling:** Zero untyped try/catch leaks in UI. All repositories return `Either<Failure, T>` using functional algebraic types (`dartz`), forcing comprehensive failure state handling at compile time.
* **Dependency Injection:** Centralized service locator (`GetIt`) registering lazy singletons for data sources, repositories, and factories for BLoCs.
* **Dynamic Multitenant & Multi-Sport Theming:** Decoupled design tokens allowing real-time theme and accent injection depending on the active sport discipline (Padel, Football, Basketball, Tennis, Pickleball).
...
* **Problem:** In high-traffic scenarios, multiple users could attempt to book the same court slot simultaneously, or concurrent POS terminals could read stale register balances, leading to double-allocation and financial discrepancy (*Race Conditions*).
* **Solution:** Replaced client-side multi-query operations with **atomic stored procedures (PL/pgSQL RPCs)** executed directly inside PostgreSQL transactions with explicit row-level locks:
  - `update_shift_expected_balance()`: Atomically updates cashier cash-in-hand without lost updates.
  - `join_open_match()` & `book_court_slot()`: Executes conditional lock checks to guarantee mutual exclusion in slot assignment.

### 2. Elimination of N+1 Query Anti-Pattern
* **Problem:** Loading monthly financial statements originally triggered independent queries for each day's transactions, causing excessive network overhead, database latency, and UI sluggishness.
...
### 3. Enterprise POS & Financial Reconciliation Engine
...
* **Blind Cash Counting & Shift Auditing:** Automated expected balance calculation based on real recorded ledger transactions versus cashier physical count, preventing cash shrinkage.
* **Hardware Optical Integration:** Real-time barcode and QR code reading integrated via camera streams (`mobile_scanner`) with haptic and audio feedback.
...
### 4. Tournament Engine & Bracket Generation
...
* **Drag & Drop Manual Seeding:** Interactive visual canvas allowing tournament organizers to manually re-seed seeded players and teams before finalizing bracket generation.
...
### 6. DevSecOps: Mobile App Hardening (RASP) & Multi-Tenant RLS
...
1. **Unit & Math Testing:** Testing deterministic pure functions (ELO calculation, bracket generation, financial rounding).
2. **BLoC State Testing:** Verifying event-to-state stream emissions under mocked failure/success conditions using `bloc_test` and `mocktail`.
...
## 👨‍💻 Author & Engineering Inquiries

Developed and architected by **[Tu Nombre Completo]** — Systems Engineer & Software Architect.

- 💼 **LinkedIn:** [linkedin.com/in/tu-perfil](https://linkedin.com/in/tu-perfil)
- 🐙 **GitHub:** [github.com/tu-usuario](https://github.com/tu-usuario)
- ✉️ **Contact:** [tu-correo-profesional@dominio.com](mailto:tu-correo-profesional@dominio.com)

---

<div align="center">
  <sub>Built with precision, Clean Architecture, and engineering rigor. © 2026 SAMO Ecosystem.</sub>
</div>
